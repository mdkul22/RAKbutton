  ...
  if (!client) {
    ...
    while(client.available())
    {
        ...
        else if (req.startsWith("/a?ssid="))
        {
          ...
          String longstr = ssid + ";" + pass + ";" + server + ";" + port + ";" + topic1 + ";" + topic2 + ";" + topic3 + ";" + topic4;
...
void beginBootUp()
{
  ...
  char topic1[s[4].length()+1];
  s[4].toCharArray(topic1, s[4].length()+1);
  char topic2[s[5].length()+1];
  s[5].toCharArray(topic2, s[5].length()+1);
  char topic3[s[6].length()+1];
  s[6].toCharArray(topic3, s[6].length()+1);
  char topic4[s[7].length()+1];
  s[7].toCharArray(topic4, s[7].length()+1);
 ...
 if (digitalRead(key1) == 0) { 
   ...
   if(client.connected()){ 
    v1=v1.equals("0")?"1":"0";
    client.publish(topic1, v1.c_str());
 } else{ 
 ...
 if (digitalRead(key2) == 0) { 
   ...
   if(client.connected()){ 
    v2=v2.equals("0")?"1":"0";
    client.publish(topic2, v2.c_str()); }
    else{ 
   Serial.println("client is not connected...") ;
 }  
   delay(500); 
 ...
 if (digitalRead(key3) == 0) { 
   ...
   if(client.connected()){ 
       v3=v3.equals("0")?"1":"0";
    client.publish(topic3, v3.c_str());
 }else{ 
   Serial.println("client is not connected...") ;
 }  
   delay(500); 
 ...
 if (digitalRead(key4) == 0) { 
   ...
   if(client.connected()){ 
    v4=v4.equals("0")?"1":"0";
    client.publish(topic4, v4.c_str());
 }else{ 
   Serial.println("client is not connected...") ;
 }     
   delay(500); 
